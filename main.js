...
function updateCartInput(e) {
  ...
  var priceElement = inputChange.getElementsByClassName('price-quantity')[0].innerText.replace('$', '');
  var subTotal = inputChange.getElementsByClassName('sub-price')[0];
  subTotal.innerText = '$' + Math.round(quantityElement.value * priceElement * 100) / 100;
...
function addToCartItems() {
  ...
  if (JSON.parse(localStorage.getItem('items')) === null || displayData === '') {
    ...
    var displayData = document.querySelector('.cart-items').innerHTML = tableData;
